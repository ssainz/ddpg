...
import numpy as np
import tflearn
from ActorCriticNetwork_cnn_util import build_cnn_network
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        # Build CNN:
        # inputs should be a shape [None, 128, 128, 4]
        # cnn_fc1 is dim shape [None, 512]
        inputs, cnn_fc1 = build_cnn_network("critic_network_cnn", True)

        #inputs = tflearn.input_data(shape=[None, self.s_dim])
        action = tflearn.input_data(shape=[None, self.a_dim])

        #w1 = tflearn.fully_connected(cnn_fc1, 300)
        #w1 = tflearn.layers.normalization.batch_normalization(w1)
        #w1 = tflearn.activations.relu(w1)

        h1 = tflearn.fully_connected(cnn_fc1, 600)
