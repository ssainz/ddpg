...
def playGame(train_indicator=1):  # 1 means Train, 0 means simply Run
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            actor_predicted = actor.target_model.predict(new_states)

            print("Actor predicted actions: ", actor_predicted.shape)
            print("New states: ", new_states.shape)

            target_q_values = critic.target_model.predict([new_states, actor_predicted])
            ...
            if (train_indicator):
                ...
                predicted_q_values = critic.model.predict([states,actions])

                # print("predicted_q_values ", predicted_q_values.shape)
                # print(predicted_q_values)

                np_loss = np.sum(np.square(y_t - predicted_q_values), axis=1) / 3
